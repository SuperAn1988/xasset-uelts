...
namespace xasset
{
    ...
    public static class PathManager
    {
        ...
        public static void GetActualPath(ref string asset)
        {
            ...
            {
                if (!manifest.GetActualPath(asset, out var value))
                {
                    continue;
                }
                asset = value;
        ...
        internal static string GetBundlePathOrURL(ManifestBundle bundle)
        {
            ...
            var containsKey = Versions.StreamingAssets.ContainsKey(assetBundleName);
            if (Versions.OfflineMode || containsKey)
            {
                if (Versions.EncryptionEnabled && Versions.UseBinary && containsKey)
                {
                    path = GetPlayerDataPath(Versions.EncryptionData);
                }
                else
                {
